...
const particles=[['에서는','에서＋는（場所＋主題）'],['에게는','에게＋는（対象＋主題）'],['으로','으로/로（手段・方向）'],['에서','에서（場所・起点）'],['에게','에게（人への対象）'],['까지','까지（〜まで）'],['부터','부터（〜から）'],['보다','보다（比較「〜より」）'],['처럼','처럼（〜のように）'],['하고','하고（〜と／〜して）'],['로','으로/로'],['이랑','이랑/랑（〜と）'],['랑','이랑/랑（〜と）'],['은','은/는（主題）'],['는','은/는（主題・連体）'],['이','이/가（主格）'],['가','이/가（主格）'],['을','을/를（目的格）'],['를','을/를（目的格）'],['에','에（場所・時・到達点）'],['도','도（〜も）'],['만','만（〜だけ）'],['의','의（〜の）']];
const common={
...
const endings=[['했어요','하다 の過去丁寧形 -했어요'],['았어요','過去丁寧形 -았어요'],['었어요','過去丁寧形 -었어요'],['고 있어요','進行形 -고 있어요'],['할 거예요','未来・予定 -(으)ㄹ 거예요'],['할게요','意思 -(으)ㄹ게요'],['해 주세요','依頼 -아/어 주세요'],['세요','尊敬・丁寧 -(으)세요'],['해요','現在丁寧形'],['아요','現在丁寧形 -아요'],['어요','現在丁寧形 -어요'],['해서','理由・順序 -아/어서'],['아서','理由・順序 -아/어서'],['어서','理由・順序 -아/어서'],['해도','譲歩・許可 -아/어도'],['고','接続 -고'],['는','現在連体形 -는'],['은','連体形 -(으)ㄴ'],['한','하다系の連体形 -(으)ㄴ'],['게','副詞化 -게']];
...
const anchor=src.lastIndexOf('PACK.push(...[{"rank":2901',readyPos);if(anchor<0)throw new Error('Rank 2901 anchor missing');
src=src.slice(0,anchor)+`PACK.push(...${JSON.stringify(cards)});\n`+src.slice(anchor);
// Visible shell moves to Daily 5000 while legacy storage namespace remains unchanged.
src=src.replaceAll('韓国語 Daily 3000','韓国語 Daily 5000').replaceAll('韓国語3000','韓国語5000').replaceAll('3000語Rank','5000語Rank');
src=src.replaceAll(' / 3000`',' / 5000`').replaceAll(' / 3000<',' / 5000<');
src=src.replace('FSRS · v1.4.27','FSRS · v1.5.0').replace('Web公開版 v1.4.27','Web公開版 v1.5.0').replace('教材化済み：Rank 1–2800','教材化済み：Rank 1–3100');
// Preserve legacy final-audit search tokens in a non-rendered comment; the established workflow will rewrite only this comment.
const compat='<!-- Daily5000 compatibility audit tokens: FSRS · v1.4.27 | Web公開版 v1.4.27 | 教材化済み：Rank 1–2800 -->';
if(!src.includes(compat))src=src.replace('</body>',`${compat}\n</body>`);
...
const newReady=src.indexOf(readyToken),newPack=Math.max(src.lastIndexOf('const PACK=',newReady),src.lastIndexOf('let PACK=',newReady));if(newPack<0||newPack>=newReady)throw new Error('PACK initialization order regression');
const extPush=src.indexOf('PACK.push(...[{"rank":3001',newPack);const oldPush=src.indexOf('PACK.push(...[{"rank":2901',extPush);if(!(newPack<extPush&&extPush<oldPush&&oldPush<newReady))throw new Error('extended PACK order regression');
const ext=src.slice(extPush,oldPush);for(const f of ['"tts_text":','"grammar_links":','"example_jp":','"word_notes":','"form_notes":'])if(ext.split(f).length-1!==100)throw new Error(`field count ${f}`);if(ext.split('文法ワンポイント：').length-1!==100)throw new Error('grammar point count');
