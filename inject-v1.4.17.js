...
const cards=require('./pack-1701-1800-v1.4.17.js');
if(cards.length!==100)throw new Error(`expected 100 cards, got ${cards.length}`);
const ranks=cards.map(c=>c.rank);
if(new Set(ranks).size!==100)throw new Error('duplicate Rank in v1.4.17');
for(let r=1701;r<=1800;r++)if(!ranks.includes(r))throw new Error(`missing Rank ${r}`);
const seen=new Set();
...
html=html.replaceAll('const APP_VERSION="1.4.16";','const APP_VERSION="1.4.17";')
         .replaceAll('FSRS · v1.4.16','FSRS · v1.4.17')
         .replaceAll('Web公開版 v1.4.16','Web公開版 v1.4.17');
html=html.replaceAll('教材化済み：Rank 1–1700','教材化済み：Rank 1–1800')
         .replaceAll('Rank 1–1700（うちRank 40','Rank 1–1800（うちRank 40');
...
for(const s of ['const STORAGE_KEY="korean-daily-3000-state-v01";','const SETTINGS_KEY="korean-daily-3000-settings-v01";','function normalizeExampleToken(','globalThis.__KOREAN_PENDING_PACKS','"rank":1800,"word":"그만두다"','const APP_VERSION="1.4.17";'])if(!html.includes(s))throw new Error(`required invariant missing: ${s}`);
...
console.log('AUDIT OK: Rank 1701-1800 mapped; 100 cards; all example tokens explained; TTS/translation/grammar links valid; duplicate examples absent; controls/events/storage/PACK invariants preserved; v1.4.17');
