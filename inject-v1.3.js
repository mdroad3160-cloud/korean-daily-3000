...
if(cards.length!==200)throw new Error(`expected 200 cards, got ${cards.length}`);
const ranks=cards.map(c=>c.rank);
if(new Set(ranks).size!==200)throw new Error('duplicate Rank in v1.4');
for(let r=501;r<=700;r++)if(!ranks.includes(r))throw new Error(`missing Rank ${r}`);
const ex=new Set();
...
for(const v of ['1.2','1.3','1.3.1','1.3.2','1.3.3','1.3.4']){
 html=html.replaceAll(`const APP_VERSION="${v}";`,'const APP_VERSION="1.4.0";');
 html=html.replaceAll(`FSRS · v${v}`,'FSRS · v1.4.0');
 html=html.replaceAll(`Web公開版 v${v}`,'Web公開版 v1.4.0');
}
html=html.replaceAll('教材化済み：Rank 1–500','教材化済み：Rank 1–700');
html=html.replaceAll('教材化済み：Rank 1–600','教材化済み：Rank 1–700');
html=html.replaceAll('Rank 1–500（うちRank 40','Rank 1–700（うちRank 40');
html=html.replaceAll('Rank 1–600（うちRank 40','Rank 1–700（うちRank 40');
...
if(!html.includes('"rank":700,"word":"채우다"'))throw new Error('Rank 700 injection failed');
if(!html.includes('const APP_VERSION="1.4.0";'))throw new Error('version bump failed');
...
console.log('AUDIT OK: Rank 501-700 mapped; Rank 601-700 morphology+grammar audited; controls wired; PACK TDZ prevented; storage keys preserved; v1.4.0');
