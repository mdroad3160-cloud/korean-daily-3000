...
const cards=require('./pack-1201-1300-v1.4.12.js');
...
if(new Set(ranks).size!==100)throw new Error('duplicate Rank in v1.4.12');
for(let r=1201;r<=1300;r++)if(!ranks.includes(r))throw new Error(`missing Rank ${r}`);
...
const dupMin=Number(process.env.DUP_MIN||1201),dupMax=Number(process.env.DUP_MAX||1300);
...
html=html.replaceAll('const APP_VERSION="1.4.11";','const APP_VERSION="1.4.12";').replaceAll('FSRS · v1.4.11','FSRS · v1.4.12').replaceAll('Web公開版 v1.4.11','Web公開版 v1.4.12');
html=html.replaceAll('教材化済み：Rank 1–1200','教材化済み：Rank 1–1300').replaceAll('Rank 1–1200（うちRank 40','Rank 1–1300（うちRank 40');
...
for(const s of ['const STORAGE_KEY="korean-daily-3000-state-v01";','const SETTINGS_KEY="korean-daily-3000-settings-v01";','function normalizeExampleToken(','globalThis.__KOREAN_PENDING_PACKS','"rank":1300,"word":"평생"','const APP_VERSION="1.4.12";'])if(!html.includes(s))throw new Error(`required invariant missing: ${s}`);
...
console.log('AUDIT OK: Rank 1201-1300 mapped; 100 cards; all example tokens explained; TTS/translation/grammar links valid; duplicate examples absent; controls/events/storage/PACK invariants preserved; v1.4.12');
