...
const cards=require('./pack-876-900-v1.4.8.js');
if(cards.length!==25)throw new Error(`expected 25 cards, got ${cards.length}`);
const ranks=cards.map(c=>c.rank);
if(new Set(ranks).size!==25)throw new Error('duplicate Rank in v1.4.8');
for(let r=876;r<=900;r++)if(!ranks.includes(r))throw new Error(`missing Rank ${r}`);
const seen=new Set();
...
html=html.replaceAll('const APP_VERSION="1.4.7";','const APP_VERSION="1.4.8";')
         .replaceAll('FSRS · v1.4.7','FSRS · v1.4.8')
         .replaceAll('Web公開版 v1.4.7','Web公開版 v1.4.8');
html=html.replaceAll('教材化済み：Rank 1–875','教材化済み：Rank 1–900')
         .replaceAll('Rank 1–875（うちRank 40','Rank 1–900（うちRank 40');
...
for(const s of ['const STORAGE_KEY="korean-daily-3000-state-v01";','const SETTINGS_KEY="korean-daily-3000-settings-v01";','function normalizeExampleToken(','"rank":900,"word":"때리다"','const APP_VERSION="1.4.8";'])if(!html.includes(s))throw new Error(`required invariant missing: ${s}`);
if(html.slice(0,html.indexOf('const PACK=')).includes('PACK.push('))throw new Error('PACK TDZ regression');
fs.writeFileSync(file,html);
console.log('AUDIT OK: Rank 876-900 mapped; all example tokens explained; Japanese translations present; grammar links valid; duplicate examples absent; controls/events/storage/PACK invariants preserved; v1.4.8');
