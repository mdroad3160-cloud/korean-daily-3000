...
const cards=require('./pack-1601-1700-v1.4.16.js');
if(cards.length!==100)throw new Error(`expected 100 cards, got ${cards.length}`);
const ranks=cards.map(c=>c.rank);
if(new Set(ranks).size!==100)throw new Error('duplicate Rank in v1.4.16');
for(let r=1601;r<=1700;r++)if(!ranks.includes(r))throw new Error(`missing Rank ${r}`);
const seen=new Set();
...
html=html.replaceAll('const APP_VERSION="1.4.15";','const APP_VERSION="1.4.16";')
         .replaceAll('FSRS · v1.4.15','FSRS · v1.4.16')
         .replaceAll('Web公開版 v1.4.15','Web公開版 v1.4.16');
html=html.replaceAll('教材化済み：Rank 1–1600','教材化済み：Rank 1–1700')
         .replaceAll('Rank 1–1600（うちRank 40','Rank 1–1700（うちRank 40');
...
for(const s of ['const STORAGE_KEY="korean-daily-3000-state-v01";','const SETTINGS_KEY="korean-daily-3000-settings-v01";','function normalizeExampleToken(','globalThis.__KOREAN_PENDING_PACKS','"rank":1700,"word":"까다"','const APP_VERSION="1.4.16";'])if(!html.includes(s))throw new Error(`required invariant missing: ${s}`);
...
console.log('AUDIT OK: Rank 1601-1700 mapped; 100 cards; all example tokens explained; TTS/translation/grammar links valid; duplicate examples absent; controls/events/storage/PACK invariants preserved; v1.4.16');
