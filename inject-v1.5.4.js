...
const all=[...R.d,...nr];
if(new Set(all.map(x=>norm(x.word))).size!==all.length)throw Error('duplicate RANK');
src=src.replace(R.m,`const RANK=${JSON.stringify(all)};`);
const grammarRanks=new Set(G.d.map(x=>Number(x.rank)));
...
src=src.replace(P.m,`const PACK=${JSON.stringify(pack)};`);
...
for(const key of ['korean-daily-3000-state-v01','korean-daily-3000-settings-v01','korean-daily-3000-sync-v01'])if(!src.includes(key))throw Error('storage/sync key missing '+key);
for(const s of ['normalizeExampleToken','PACK','READY','function initCloudSync(','function syncCloudCore(','function queueCloudSave('])if(!src.includes(s))throw Error('core marker missing '+s);
const packPos=src.search(/(?:const|let) PACK=/),readyPos=src.indexOf('const READY=new Map(PACK.filter');if(!(packPos>=0&&readyPos>packPos))throw Error('PACK/READY initialization order invalid');
