...
const cards=require('./pack-901-1000-v1.4.9.js');

if(cards.length!==100)throw new Error(`expected 100 cards, got ${cards.length}`);
const ranks=cards.map(c=>c.rank);
if(new Set(ranks).size!==100)throw new Error('duplicate Rank in v1.4.9');
for(let r=901;r<=1000;r++)if(!ranks.includes(r))throw new Error(`missing Rank ${r}`);

const seen=new Set();
for(const c of cards){
 for(const k of ['word','meaning','example','example_jp','grammar_jp','form_notes','register','tts_text'])
   if(!c[k])throw new Error(`Rank ${c.rank}: missing ${k}`);
...
const rankRows=JSON.parse(rm[1]);
...
const grammarRows=JSON.parse(gm[1]);
...
html=html.replaceAll('const APP_VERSION="1.4.8";','const APP_VERSION="1.4.9";')
         .replaceAll('FSRS · v1.4.8','FSRS · v1.4.9')
         .replaceAll('Web公開版 v1.4.8','Web公開版 v1.4.9');
html=html.replaceAll('教材化済み：Rank 1–900','教材化済み：Rank 1–1000')
         .replaceAll('Rank 1–900（うちRank 40','Rank 1–1000（うちRank 40');
...
const eventChecks=[
 'startBtn").addEventListener','showAnswerBtn").addEventListener','speakWordBtn").addEventListener',
 'speakExampleBtn").addEventListener','knownBtn").addEventListener','[data-grade]',
 'rankSearch").addEventListener','rankLimit").addEventListener','saveSettingsBtn").addEventListener',
 'exportBtn").addEventListener','importProgress").addEventListener','importPack").addEventListener'
];
...
const packPos=html.indexOf('const PACK=');
if(packPos<0)throw new Error('PACK declaration missing');
if(html.slice(0,packPos).includes('PACK.push('))throw new Error('PACK TDZ regression');

fs.writeFileSync(file,html);
console.log('AUDIT OK: Rank 901-1000 mapped; 100 cards; all example tokens explained; TTS/translation/grammar links valid; duplicate examples absent; controls/events/storage/PACK invariants preserved; v1.4.9');
