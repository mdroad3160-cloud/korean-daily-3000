...
const cards=require('./pack-1101-1200-v1.4.11.js');
...
if(new Set(ranks).size!==100)throw new Error('duplicate Rank in v1.4.11');
for(let r=1101;r<=1200;r++)if(!ranks.includes(r))throw new Error(`missing Rank ${r}`);

const seen=new Set();
for(const c of cards){
 for(const k of ['word','meaning','example','example_jp','grammar_jp','form_notes','register','tts_text']) if(!c[k])throw new Error(`Rank ${c.rank}: missing ${k}`);
...
const dupMin=Number(process.env.DUP_MIN||1101),dupMax=Number(process.env.DUP_MAX||1200);
...
html=html.replaceAll('const APP_VERSION="1.4.10";','const APP_VERSION="1.4.11";').replaceAll('FSRS · v1.4.10','FSRS · v1.4.11').replaceAll('Web公開版 v1.4.10','Web公開版 v1.4.11');
html=html.replaceAll('教材化済み：Rank 1–1100','教材化済み：Rank 1–1200').replaceAll('Rank 1–1100（うちRank 40','Rank 1–1200（うちRank 40');
...
for(const s of ['const STORAGE_KEY="korean-daily-3000-state-v01";','const SETTINGS_KEY="korean-daily-3000-settings-v01";','function normalizeExampleToken(','globalThis.__KOREAN_PENDING_PACKS','"rank":1200,"word":"자동차"','const APP_VERSION="1.4.11";'])if(!html.includes(s))throw new Error(`required invariant missing: ${s}`);
...
console.log('AUDIT OK: Rank 1101-1200 mapped; 100 cards; all example tokens explained; TTS/translation/grammar links valid; duplicate examples absent; controls/events/storage/PACK invariants preserved; v1.4.11');
