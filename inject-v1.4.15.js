...
if(new Set(ranks).size!==100)throw new Error('duplicate Rank in v1.4.15');
for(let r=1501;r<=1600;r++)if(!ranks.includes(r))throw new Error(`missing Rank ${r}`);
const seen=new Set();
...
const dupMin=Number(process.env.DUP_MIN||1501),dupMax=Number(process.env.DUP_MAX||1600);
...
html=html.replaceAll('const APP_VERSION="1.4.14";','const APP_VERSION="1.4.15";')
         .replaceAll('FSRS · v1.4.14','FSRS · v1.4.15')
         .replaceAll('Web公開版 v1.4.14','Web公開版 v1.4.15');
html=html.replaceAll('教材化済み：Rank 1–1500','教材化済み：Rank 1–1600')
         .replaceAll('Rank 1–1500（うちRank 40','Rank 1–1600（うちRank 40');
...
console.log('AUDIT OK: Rank 1501-1600 mapped; 100 cards; all example tokens explained; TTS/translation/grammar links valid; duplicate examples absent; controls/events/storage/PACK invariants preserved; v1.4.15');
