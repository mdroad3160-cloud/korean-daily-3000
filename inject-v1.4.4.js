...
const cards=require('./pack-776-800-v1.4.4.js');
if(cards.length!==25)throw new Error(`expected 25 cards, got ${cards.length}`);
for(let r=776;r<=800;r++)if(!cards.some(c=>c.rank===r))throw new Error(`missing Rank ${r}`);
const seen=new Set();
...
html=html.replaceAll('const APP_VERSION="1.4.3";','const APP_VERSION="1.4.4";')
         .replaceAll('FSRS · v1.4.3','FSRS · v1.4.4')
         .replaceAll('Web公開版 v1.4.3','Web公開版 v1.4.4');
html=html.replaceAll('教材化済み：Rank 1–775','教材化済み：Rank 1–800')
         .replaceAll('Rank 1–775（うちRank 40','Rank 1–800（うちRank 40');
...
for(const s of ['const STORAGE_KEY="korean-daily-3000-state-v01";','const SETTINGS_KEY="korean-daily-3000-settings-v01";','function normalizeExampleToken(','"rank":800,"word":"국민"','const APP_VERSION="1.4.4";'])if(!html.includes(s))throw new Error(`required invariant missing: ${s}`);
if(html.slice(0,html.indexOf('const PACK=')).includes('PACK.push('))throw new Error('PACK TDZ regression');
fs.writeFileSync(file,html);
console.log('AUDIT OK: Rank 776-800 mapped; all example tokens explained; grammar links valid; duplicate examples absent; controls/storage/PACK invariant preserved; v1.4.4');
