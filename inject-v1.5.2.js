'use strict';
const fs=require('fs');
const START=3201,END=3300,VERSION='1.5.2';
const file=process.argv[2]||'_site/index.html';
let src=fs.readFileSync(file,'utf8');
const candidates=require('./pack-3201-3300-v1.5.2.js');
...
function noteToken(t){const c=clean(t);for(const p of particles)if(c.endsWith(p)&&c.length>p.length)return `${c}：${c.slice(0,-p.length)}＋助詞 ${p}`;if(/했어요$/.test(c))return `${c}：하다系用言の過去丁寧形 -했어요`;if(/았어요$|었어요$/.test(c))return `${c}：用言の過去丁寧形 -았/었어요`;if(/해요$|아요$|어요$/.test(c))return `${c}：用言の現在丁寧形 -아/어요`;if(/해야 해요$/.test(c))return `${c}：義務 -아/어야 하다`;if(/할 거예요$/.test(c))return `${c}：予定・推測 -(으)ㄹ 거예요`;if(/세요$/.test(c))return `${c}：丁寧な依頼・命令 -(으)세요`;if(/해서$|아서$|어서$/.test(c))return `${c}：理由・順序 -아/어서`;return `${c}：例文中の語。日本語訳の対応箇所と合わせて意味・用法を確認`;}
...
for(const c of cards){if(!c.tts||!c.jp||!c.meaning||!Object.keys(c.form_notes).length)throw Error('card field missing '+c.rank);for(const t of c.example.split(/\s+/).map(clean).filter(Boolean))if(!c.form_notes[t])throw Error('unexplained '+c.rank+':'+t);}
