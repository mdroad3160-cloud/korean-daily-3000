...
const cards=require('./pack-826-850-v1.4.6.js');
if(cards.length!==25)throw new Error(`expected 25 cards, got ${cards.length}`);
for(let r=826;r<=850;r++)if(!cards.some(c=>c.rank===r))throw new Error(`missing Rank ${r}`);
const seen=new Set();
...
html=html.replaceAll('const APP_VERSION="1.4.5";','const APP_VERSION="1.4.6";')
         .replaceAll('FSRS · v1.4.5','FSRS · v1.4.6')
         .replaceAll('Web公開版 v1.4.5','Web公開版 v1.4.6');
html=html.replaceAll('教材化済み：Rank 1–825','教材化済み：Rank 1–850')
         .replaceAll('Rank 1–825（うちRank 40','Rank 1–850（うちRank 40');
...
const eventChecks=['showAnswerBtn").addEventListener','speakWordBtn").addEventListener','speakExampleBtn").addEventListener','knownBtn").addEventListener','[data-grade]','rankSearch").addEventListener','rankLimit").addEventListener','saveSettingsBtn").addEventListener','exportBtn").addEventListener','importProgress").addEventListener','importPack").addEventListener'];
for(const s of eventChecks)if(!html.includes(s))throw new Error(`event binding missing: ${s}`);
for(const s of ['const STORAGE_KEY="korean-daily-3000-state-v01";','const SETTINGS_KEY="korean-daily-3000-settings-v01";','function normalizeExampleToken(','"rank":850,"word":"살펴보다"','const APP_VERSION="1.4.6";'])if(!html.includes(s))throw new Error(`required invariant missing: ${s}`);
if(html.slice(0,html.indexOf('const PACK=')).includes('PACK.push('))throw new Error('PACK TDZ regression');
fs.writeFileSync(file,html);
console.log('AUDIT OK: Rank 826-850 mapped; all example tokens explained; grammar links valid; duplicate examples absent; controls/events/storage/PACK invariant preserved; v1.4.6');
